...
export async function searchQuickbooksTimeActivities(data) {
    try {
        await quickbooksClient.authenticate();
        const quickbooks = quickbooksClient.getQuickbooks();
        const criteria = {};
        if (data.employee_ref)
            criteria.EmployeeRef = data.employee_ref;
        if (data.vendor_ref)
        ...
        return new Promise((resolve) => {
            quickbooks.findTimeActivities(criteria, (err, result) => {
                if (err)
                    resolve({ result: null, isError: true, error: formatError(err) });
                else
                    resolve({ result: result?.QueryResponse?.TimeActivity || [], isError: false, error: null });
            });
        });
    }
    catch (error) {
