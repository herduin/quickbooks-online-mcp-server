...
const toolHandler = async ({ params }) => {
    ...
    return { content: [{ type: "text", text: `Deposit created:` }, { type: "text", text: JSON.stringify(response.result, null, 2) }] };
};
export const CreateDepositTool = { name: toolName, description: toolDescription, schema: toolSchema, handler: toolHandler };
