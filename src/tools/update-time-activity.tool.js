...
const toolHandler = async ({ params }) => {
    ...
    return { content: [{ type: "text", text: `Time activity updated:` }, { type: "text", text: JSON.stringify(response.result, null, 2) }] };
};
export const UpdateTimeActivityTool = { name: toolName, description: toolDescription, schema: toolSchema, handler: toolHandler };
