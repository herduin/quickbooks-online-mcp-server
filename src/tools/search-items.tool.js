...
const filterSchema = z.object({
    ...
    if (!isValidItemValueType(obj.field, obj.value)) {
        ctx.addIssue({
...
const toolHandler = async ({ params }) => {
    ...
    if (response.isError) {
        return { content: [{ type: "text", text: `Error searching items: ${response.error}` }] };
    }
    const items = response.result;
    return {
        content: [
            { type: "text", text: `Found ${items?.length || 0} items` },
            ...(items?.map((item) => ({ type: "text", text: JSON.stringify(item) })) || []),
        ],
    };
};
function isValidItemValueType(field, value) {
    const expected = ITEM_FIELD_TYPE_MAP[field];
    if (!expected)
        return true;
    const check = (v) => {
        switch (expected) {
