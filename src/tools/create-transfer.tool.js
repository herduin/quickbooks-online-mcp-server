...
const toolHandler = async ({ params }) => {
    ...
    return { content: [{ type: "text", text: `Transfer created:` }, { type: "text", text: JSON.stringify(response.result, null, 2) }] };
};
export const CreateTransferTool = { name: toolName, description: toolDescription, schema: toolSchema, handler: toolHandler };
