...
const toolHandler = async ({ params }) => {
    ...
    return { content: [{ type: "text", text: `Time activity created:` }, { type: "text", text: JSON.stringify(response.result, null, 2) }] };
};
export const CreateTimeActivityTool = { name: toolName, description: toolDescription, schema: toolSchema, handler: toolHandler };
