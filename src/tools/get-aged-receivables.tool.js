...
const toolHandler = async ({ params }) => {
    ...
    return { content: [{ type: "text", text: `Aged Receivables Report:` }, { type: "text", text: JSON.stringify(response.result, null, 2) }] };
};
export const GetAgedReceivablesTool = { name: toolName, description: toolDescription, schema: toolSchema, handler: toolHandler };
