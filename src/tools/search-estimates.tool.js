...
]).describe("Field to filter on – must be a property of the QuickBooks Online Estimate entity.");
...
export const SearchEstimatesTool = {
    ...
    handler: async (args) => {
        ...
        if (response.isError) {
            return {
                content: [{ type: "text", text: `Error searching estimates: ${response.error}` }],
            };
        }
        return {
            content: [
                { type: "text", text: Array.isArray(response.result) ? `Found ${response.result.length} estimates:` : `Count: ${response.result}` },
                ...(Array.isArray(response.result)
                    ? response.result.map((e) => ({ type: "text", text: JSON.stringify(e) }))
                    : []),
            ],
        };
    },
