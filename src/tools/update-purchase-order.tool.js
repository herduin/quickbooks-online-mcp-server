...
const toolHandler = async ({ params }) => {
    ...
    if (response.isError) {
        return { content: [{ type: "text", text: `Error updating purchase order: ${response.error}` }] };
    }
    return {
        content: [
            { type: "text", text: `Purchase order updated successfully:` },
            { type: "text", text: JSON.stringify(response.result, null, 2) },
        ],
    };
};
export const UpdatePurchaseOrderTool = {
    name: toolName,
