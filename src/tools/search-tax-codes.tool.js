...
const toolHandler = async ({ params }) => {
    ...
    return { content: [{ type: "text", text: `Found ${response.result.length} tax codes:` }, { type: "text", text: JSON.stringify(response.result, null, 2) }] };
};
export const SearchTaxCodesTool = { name: toolName, description: toolDescription, schema: toolSchema, handler: toolHandler };
