...
export const SearchVendorsTool = {
    ...
    handler: async (args) => {
        ...
        return {
            content: [
                { type: "text", text: Array.isArray(response.result) ? `Found ${response.result.length} vendors:` : `Count: ${response.result}` },
                ...(Array.isArray(response.result)
                    ? response.result.map((v) => ({ type: "text", text: JSON.stringify(v) }))
                    : []),
            ],
        };
    },
