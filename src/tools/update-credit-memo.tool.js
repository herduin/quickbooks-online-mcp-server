...
const toolHandler = async ({ params }) => {
    ...
    if (response.isError) {
        return { content: [{ type: "text", text: `Error updating credit memo: ${response.error}` }] };
    }
    return {
        content: [
            { type: "text", text: `Credit memo updated successfully:` },
            { type: "text", text: JSON.stringify(response.result, null, 2) },
        ],
    };
};
export const UpdateCreditMemoTool = {
    name: toolName,
