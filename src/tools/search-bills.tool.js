...
const billFieldEnum = z.enum([
    ...
    "DepartmentRef",
    "CurrencyRef"
]).describe("Field to filter on – must be a property of the QuickBooks Online Bill entity.");
...
const toolSchema = z.object({
    ...
    criteria: z
        ...
        .optional()
        .describe("Filters to apply. Use the advanced form {field,value,operator?} for operators or the simple {key,value} pairs."),
    ...
    handler: async (args) => {
        ...
        if (Array.isArray(criteria) && criteria.length > 0) {
            ...
            if (typeof first === "object" && "field" in first) {
                ...
                criteriaToSend = criteria.reduce((acc, { key, value }) => {
        ...
        if (response.isError) {
            return {
                content: [{ type: "text", text: `Error searching bills: ${response.error}` }],
            };
        }
        return {
            content: [
                { type: "text", text: Array.isArray(response.result) ? `Found ${response.result.length} bills:` : `Count: ${response.result}` },
                ...(Array.isArray(response.result)
                    ? response.result.map((b) => ({ type: "text", text: JSON.stringify(b) }))
                    : []),
            ],
        };
    },
